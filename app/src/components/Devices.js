export default [{
  'name': 'Macbook Pro - Chrome 55',
  'parse': 247.844,
  'eval': 13.65,
  'image': './static/macbook-pro.jpg'
}, {
  'name': 'Macbook Pro - Safari 10',
  'parse': 69.914,
  'eval': 173.93,
  'image': './static/macbook-pro.jpg'
}, {
  'name': 'Moto G4 - Chrome 55',
  'parse': 1429,
  'eval': 73,
  'image': './static/motog4.jpg'
}, {
  'name': 'Nexus 5X - Chrome 55',
  'parse': 1038.965,
  'eval': 55.339,
  'image': './static/nexus5x.jpg'
}, {
  'name': 'Nexus 5 - Chrome 55',
  'parse': 1834.46,
  'eval': 200.435,
  'image': './static/nexus5.jpg'
}, {
  'name': 'iPhone 7 - Safari',
  'parse': 76.78,
  'eval': 72,
  'image': './static/iphone-7.jpg'
}, {
  'name': 'iPhone 6 - Safari',
  'parse': 645,
  'eval': 181,
  'image': './static/iphone-6.jpg'
}, {
  'name': 'iPhone 5c - Safari',
  'parse': 1429,
  'eval': 71,
  'image': './static/iphone-5c.jpg'
}, {
  'name': 'PixelXL - Chrome 55',
  'parse': 1114,
  'eval': 40,
  'image': './static/pixelxl.jpg'
}, {
  'name': 'Moto E - Chrome 55',
  'parse': 2806.21,
  'eval': 141.089,
  'image': './static/moto-e.jpg'
}, {
  'name': 'Galaxy S7 Edge - Chrome 55',
  'parse': 956.16,
  'eval': 31.654,
  'image': './static/galaxy-s7.jpg'
}, {
  'name': 'Galaxy S6 Edge - Chrome 55',
  'parse': 1141.63,
  'eval': 42.649,
  'image': './static/galaxy-s6.jpg'
}, {
  'name': 'Galaxy S5 - Chrome 55',
  'parse': 2224.495,
  'eval': 75.909,
  'image': './static/galaxy-s5.jpg'
}, {
  'name': 'Moto G1 - Chrome 55',
  'parse': 2936.944,
  'eval': 169.385,
  'image': './static/motog-gen1.jpg'
}, {
  'name': 'Nexus 7 - Chrome 55',
  'parse': 3005.095,
  'eval': 115.724,
  'image': './static/nexus-7.jpg'
}, {
  'name': 'Thinkpad T430 - Chrome 55',
  'parse': 290.51,
  'eval': 40.545,
  'image': './static/thinkpad-t430.jpg'
}, {
  'name': 'Thinkpad T430 - Firefox 50',
  'parse': 165.205,
  'eval': 7.634,
  'image': './static/thinkpad-t430.jpg'
}, {
  'name': 'Thinkpad T430 - Microsoft Edge',
  'parse': 384.795,
  'eval': 1955.314,
  'image': './static/thinkpad-t430.jpg'
}, {
  'name': 'Android One Airtel - Chrome 55',
  'parse': 3092.525,
  'eval': 140.824,
  'image': './static/android-one.jpg'
}, {
  'name': 'Google Nexus 6P - Chrome 55',
  'parse': 1273,
  'eval': 612,
  'image': './static/nexus-6p.jpg'
}, {
  'name': 'Samsung Galaxy Note II - Chrome 55',
  'parse': 6051,
  'eval': 1916,
  'image': './static/samsung-galaxy-note-ii.jpg'
},
{
  'name': 'Motorola XT1097 Moto X (2nd Gen) - Chrome 55',
  'parse': 2314,
  'eval': 427,
  'image': './static/moto-x-1097-2nd-gen.jpg'
},
{
  'name': 'Motorola XT1052 Moto X - Chrome 55',
  'parse': 2828,
  'eval': 149,
  'image': './static/moto-x-1052.jpg'
},
{
  'name': 'Sony D5803 Xperia Z3 Compact (WebView)',
  'parse': 4677,
  'eval': 140,
  'image': './static/sony-xperia-z3.jpg'
},
{
  'name': 'SonyEricsson C6603 Xperia Z (WebView)',
  'parse': 3745,
  'eval': 157,
  'image': './static/sony-xperia-z3.jpg'
},
{
  'name': 'Nexus 4 - Chrome 55',
  'parse': 4663,
  'eval': 382,
  'image': './static/nexus4.jpg'
},
{
  'name': 'Huawei Ascend Mate 7 - Chrome 55',
  'parse': 3784,
  'eval': 187,
  'image': './static/ascend-mate-7.jpg'
},
{
  'name': 'Samsung Galaxy Note 4 - Chrome 55',
  'parse': 2473,
  'eval': 92,
  'image': './static/galaxy-note-4.jpg'
},
{
  'name': 'Motorola MotoG3 - Chrome',
  'parse': 1761,
  'eval': 205,
  'image': './static/moto-g3.jpg'
},
{
  'name': 'LG G4 - Chrome 55',
  'parse': 2245,
  'eval': 211,
  'image': './static/lh-h815-g4.jpg'
},
{
  'name': 'Lenovo A6000 - Chrome',
  'parse': 2161,
  'eval': 118,
  'image': './static/lenovo-a6000.jpg'
},
{
  'name': 'Huawei P9 Plus - Chrome',
  'parse': 1511,
  'eval': 438,
  'image': './static/huawei-p9.jpg'
},
{
  'name': 'Samsung Galaxy A3 - Chrome',
  'parse': 2326,
  'eval': 291,
  'image': './static/galaxy-a3.jpg'
}, {
  'name': 'OnePlus 3 - Chrome',
  'parse': 1254,
  'eval': 44,
  'image': './static/oneplus-3.jpg'
}, {
  'name': 'Xiaomi Mi 4i - Chrome',
  'parse': 2167,
  'eval': 54,
  'image': './static/xiaomi-mi-41.jpg'
}, {
  'name': 'Asus T00J Zenfone 5 - Chrome',
  'parse': 6574,
  'eval': 1346,
  'image': './static/asus-zenfone-5.jpg'
}, {
  'name': 'Microsoft 640 Lumia 640 LTE - Edge',
  'parse': 2377,
  'eval': 186,
  'image': './static/ms-640-lumia.jpg'
}, {
  'name': 'Apple iPhone 6s Plus',
  'parse': 391,
  'eval': 101,
  'image': './static/iphone-6s-plus.jpg'
}, {
  'name': 'Apple iPhone 6s',
  'parse': 408,
  'eval': 101,
  'image': './static/iphone-6s.jpg'
}, {
  'name': 'Apple iPhone 5s',
  'parse': 1319,
  'eval': 402,
  'image': './static/iphone-5s.jpg'
}]
